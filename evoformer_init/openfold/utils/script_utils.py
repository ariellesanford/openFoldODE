...
#
#     return out
def run_model(model, batch, tag, output_dir, save_intermediates=False, save_dir=None):
    with torch.no_grad():
        # Handle optional template masking
        template_enabled = model.config.template.enabled
        ...
        t = time.perf_counter()
        out = model(batch, save_intermediates_dir=save_dir)
        inference_time = time.perf_counter() - t
        logger.info(f"Inference time: {inference_time}")

        update_timings({tag: {"inference": inference_time}}, os.path.join(output_dir, "timings.json"))

        # Reset template config to original
        model.config.template.enabled = template_enabled

        # === Save Evoformer intermediates ===
        evoformer = getattr(model, "evoformer", None)

        if evoformer and hasattr(evoformer, "recycle_intermediates") and isinstance(evoformer.recycle_intermediates, list):
            base_save_dir = os.path.join(output_dir, f"{tag}_evoformer_blocks")
            logger.info(f"💾 Saving Evoformer intermediates for {len(evoformer.recycle_intermediates)} recycle steps...")

            for r_idx, recycle_blocks in enumerate(evoformer.recycle_intermediates):
                recycle_dir = os.path.join(base_save_dir, f"recycle_{r_idx}")
                os.makedirs(recycle_dir, exist_ok=True)
                for i, (m_block, z_block) in enumerate(recycle_blocks):
                    torch.save(m_block, os.path.join(recycle_dir, f"m_block_{i}.pt"))
                    torch.save(z_block, os.path.join(recycle_dir, f"z_block_{i}.pt"))
                logger.info(f"Saved {len(recycle_blocks)} blocks for recycle step {r_idx} in {recycle_dir}")
        else:
            logger.warning("⚠️ No Evoformer intermediates found or 'recycle_intermediates' is missing.")
