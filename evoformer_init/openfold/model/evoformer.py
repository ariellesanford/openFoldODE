...
class EvoformerStack(nn.Module):
    ...
    def _prep_blocks(self,
        ...
        def wrap_block(block):
            def wrapped(m, z):
                ...
                if save_intermediates and hasattr(self, "intermediates"):
                    self.intermediates.append((m_out.detach().cpu(), z_out.detach().cpu()))
                return m_out, z_out

            return wrapped

        blocks = [wrap_block(b) for b in self.blocks]

        if self.clear_cache_between_blocks:
            def block_with_cache_clear(block):
            ...
            blocks = [block_with_cache_clear(b) for b in blocks]
    ...
    def forward(self,
                ...
                inplace_safe: bool = False,
                _mask_trans: bool = True,
                recycle_step: Optional[int] = None,
                save_dir: Optional[str] = None,
                ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Evoformer forward pass with optional per-recycle saving.
        """

        save_intermediates = save_dir is not None and recycle_step is not None

        if save_intermediates and hasattr(self, "intermediates"):
            self.intermediates = []
            self.intermediates.append(
                (m.detach().cpu().clone(), z.detach().cpu().clone())
            )
            print(f"[DEBUG] Appended initial input: m={m.shape}, z={z.shape}, mean m={m.mean().item():.4f}")

        # ✅ Add early return here
        if save_dir is not None and recycle_step is not None:
            print("initial input returned")
            import os
            recycle_path = os.path.join(save_dir, f"recycle_{recycle_step}")
            os.makedirs(recycle_path, exist_ok=True)
            for i, (m_block, z_block) in enumerate(self.intermediates):
                torch.save(m_block, os.path.join(recycle_path, f"m_block_{i}.pt"))
                torch.save(z_block, os.path.join(recycle_path, f"z_block_{i}.pt"))
            torch.save(msa_mask.detach().cpu(), os.path.join(recycle_path, "msa_mask.pt"))
            torch.save(pair_mask.detach().cpu(), os.path.join(recycle_path, "pair_mask.pt"))

        # ✅ Fully stop the model before Evoformer begins
        print("The model is stopping now")
        return m, z, self.linear(m[..., 0, :, :])
        ...
        if save_intermediates and hasattr(self, "intermediates"):
            if hasattr(self, "recycle_intermediates"):
                self.recycle_intermediates.append(list(self.intermediates))
