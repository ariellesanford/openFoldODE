...
class EvoformerStack(nn.Module):
    ...
    def forward(self,
        ...
        if hasattr(self, "intermediates"):
            self.intermediates = []
            self.intermediates.append(  # Save initial input
        ...
        if hasattr(self, "intermediates"):
            # self.intermediates.append(  # Save final output
            #     (m.detach().cpu().clone(), z.detach().cpu().clone())
            # )
            # print(f"[DEBUG] Appended final output: m={m.shape}, z={z.shape}, mean m={m.mean().item():.4f}")
