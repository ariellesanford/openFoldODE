...
#         model.config.template.enabled = template_enabled
#
#         # 🔽 Save Evoformer block outputs (if available)
#         evoformer = getattr(model, "evoformer_stack", None)
#         if evoformer is None and hasattr(model, "trunk"):
#             evoformer = getattr(model.trunk, "evoformer_stack", None)
#
#         if evoformer and hasattr(evoformer, "intermediates"):
#             save_dir = os.path.join(output_dir, f"{tag}_evoformer_blocks")
#             os.makedirs(save_dir, exist_ok=True)
#
#             for i, (m_block, z_block) in enumerate(evoformer.intermediates):
#                 torch.save(m_block, os.path.join(save_dir, f"m_block_{i}.pt"))
#                 torch.save(z_block, os.path.join(save_dir, f"z_block_{i}.pt"))
#             logger.info(f"Saved Evoformer intermediates to {save_dir}")
#
#     return out
def run_model(model, batch, tag, output_dir):
    with torch.no_grad():
        ...
        # === DEBUG: Check if first and second Evoformer intermediates are the same ===
        if hasattr(model, "evoformer_stack") and hasattr(model.evoformer_stack, "intermediates"):
            if len(model.evoformer_stack.intermediates) > 1:
                m0, _ = model.evoformer_stack.intermediates[0]
                m1, _ = model.evoformer_stack.intermediates[1]
                identical = torch.allclose(m0, m1)
                print(f"[DEBUG] m0 vs m1 identical: {identical}")
            else:
                print("[DEBUG] Not enough Evoformer intermediates to compare.")
        ...
        # === DEBUGGING START ===
        logger.info(f"Model modules: {list(model._modules.keys())}")

        evoformer = None
        if hasattr(model, "evoformer"):
            logger.info("✅ Found evoformer directly in model.")
            evoformer = model.evoformer
        else:
            logger.warning("⚠️ Could not find evoformer in model.")

        if evoformer is not None:
            if hasattr(evoformer, "intermediates"):
                logger.info(f"✅ evoformer has 'intermediates' attribute of type {type(evoformer.intermediates)}")
                logger.info(f"✅ Number of saved blocks: {len(evoformer.intermediates)}")
            else:
                logger.warning("⚠️ evoformer has no 'intermediates' attribute.")

        if evoformer and hasattr(evoformer, "intermediates") and len(evoformer.intermediates) > 0:
            logger.info("💾 Saving Evoformer intermediates...")
            save_dir = os.path.join(output_dir, f"{tag}_evoformer_blocks")
            os.makedirs(save_dir, exist_ok=True)

            for i, (m_block, z_block) in enumerate(evoformer.intermediates):
                m_path = os.path.join(save_dir, f"m_block_{i}.pt")
                z_path = os.path.join(save_dir, f"z_block_{i}.pt")
                torch.save(m_block, m_path)
                torch.save(z_block, z_path)
                logger.info(f"Saved block {i}: {m_path}, {z_path}")

            logger.info(f"✅ Finished saving {len(evoformer.intermediates)} Evoformer blocks to {save_dir}")
        else:
            logger.warning("⚠️ No Evoformer intermediates were saved — 'intermediates' is empty or not found.")
        # === DEBUGGING END ===
